# Regulated Advisory Platform (Provenance-First)

- Source: internal product exploration; builds on the regulated-document workflow proof (see
  `2026-04-25-regulated-document-workflows.md`).
...
- L1: raw documents → verified regulated facts.
- L2: verified facts → derived compliance positions (this eligibility item applies, this benefit qualifies,
  effective rate = X), each citing facts + the governing rule + the calculation.
- L3: positions → strategy ("shift $12k to save $2,100," "you are over-retained amount
  by $4k"), each citing the positions it rests on.

The differentiator is end-to-end lineage plus derived-trust invalidation: change the source document and
every downstream position and recommendation auto-flags as stale, and a user or auditor can
drill from a recommendation down to the source-document line and the official source rule.
...
- **Embedded filing** (embedded providers integrated into financial products):
...
- **Pro planning** (Holistiplan, Corvee, Keeper; retirement: MaxiFi, Boldin, Income Lab):
  ...
  strategy recompute when income changes.
- **AI regulated assistants**: already advertise citation to official codes,
...
- "Cite the official source rule" — already done by several advisory assistants.
...
source-document line (source document Box 1) → verified fact → derived position → recommendation, such
that changing the fact auto-stales the recommendation. AI tools cite the rule but not your
source document; planners recompute but are black-box on lineage; embedded players ingest data but do not
...
- Market: US compliance-prep services ~$34.9B (2025), ~5.8% CAGR.
- Regulatory tailwind: official source Direct File cancelled for 2026 — the free-gov-filing threat
  receded.
- The wedge-shaped pain: AI regulated distrust. ~50% of complex AI regulated advice estimated wrong; no
  official source safe harbor; the filer/preparer owns the error. IRC §6694 preparer penalties ($635/return
...
not a consumer headline. Consumers do not drill into official source code (94% Direct File satisfaction
was about simplicity). The honest demand mechanism: the AI-advisory wave created a
hallucination/liability crisis, and provenance is the credible answer to "can I trust this
enough to put my name on it."

Sharpest initial buyer: **RIAs/wealth managers, then reviewers/EAs** — not consumers. RIAs already
pay for compliance-aware planning (Holistiplan/Boldin proved it), make strategy-grade recommendations
...
- The hard part is the data spine, not the UX: reliable document→fact extraction (Holistiplan
  ...
  official source rules. This is also the moat.
- Provenance ≠ correctness: a verifiable chain to a wrong extracted fact is still wrong.
- Channel: embedded providers own distribution; incumbents own the filing relationship.
...
- **Consumes Survey** for L1: document ingestion, form extraction, fact resolution, and the
  human-review loop. The regulated-form extractors are domain plugins; the resolution/review machinery
  is shared.
...
- **Owns** (the actual product): the official-rule spine (machine-linkable, annually maintained), the
  domain derivation logic (positions, strategies), materiality calibration for regulated dollars,
  and the advisor-facing review and reporting experience.

The moat lives in what it owns — the rule spine and domain logic — not in the foundation it
consumes. See `2026-04-25-regulated-document-workflows.md` for the existing claim/evidence modeling this builds on.

## Follow-up questions

- Can the advisor-facing "show-your-work + auto-stale-flagging" slice ship before the full strategy
  engine?
- How much of the official-rule spine can be sourced from the existing managed-rules work?
- What is the minimum derivation depth that demonstrates auto-stale value to an advisor?
